...
int main() {
    ...
    rec->log_static("/field/part", rerun::LineStrips3D(rerun::components::LineStrip3D(enu_pts))
                                       .with_colors({{rerun::Color(70, 120, 70)}})
                                       .with_radii({{0.2f}}));

    auto geo_headland = rerun::components::GeoLineString::from_lat_lon(wgs_pts);
    rec->log_static("/field/part", rerun::archetypes::GeoLineStrings(geo_headland)
                                       .with_colors({{rerun::Color(70, 120, 70)}})
                                       .with_radii({{0.5f}}));
